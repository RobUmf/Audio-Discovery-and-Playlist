...
def run_ffmpeg(input_file, output_file, bitrate, include_cover, cover_size="300"):
    ...
    cmd.extend([
        ...
        '-b:a', bitrate, '-ar', '44100', str(output_file)
    ])
    try:
        ...
        if include_cover:
            fallback_cmd = [
                'ffmpeg', '-y', '-hide_banner', '-loglevel', 'error', '-i', str(input_file),
...
def process_track(f, input_root, output_root, flat, bitrate, cover, cover_size, override_group):
    ...
    if success:
        ...
        row_data = {
            'original_path': str(f.expanduser()),
            'processed_path': str(target_path.expanduser()),
            'filename': f.name,
            'grouping': group_name,
            'file_path': str(target_path.expanduser()),
            'format': meta['format'],
...
if __name__ == "__main__":
    REPO_DIR = Path(__file__).parent.expanduser()
    BASE_DIR = Path(os.environ.get("MUSIC_BASE", REPO_DIR.parent)).expanduser()
    ...
    input_root = Path(args.input or config.get("input_dir", BASE_DIR / "musicraw")).expanduser()
    output_root = Path(args.output or config.get("output_dir", BASE_DIR / "128mp3")).expanduser()
    db_path = Path(args.db or config.get("db_path", REPO_DIR / "audio_database.db")).expanduser()
    ...
    for f in files_to_process:
        ...
        expected_target = str((output_root / test_rel.with_suffix('.mp3')).expanduser())
    ...
    with ProcessPoolExecutor(max_workers=workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            if success and row_data:
                ...
                update_clause = ", ".join([f"{key} = excluded.{key}" for key in keys if key != "file_path"])
                sql = f"INSERT INTO tracks ({columns}) VALUES ({placeholders}) ON CONFLICT(file_path) DO UPDATE SET {update_clause}"
    ...
    with ProcessPoolExecutor(max_workers=workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            if success and row_data:
                ...
                # Build the dynamic update clause for the UPSERT, excluding the unique conflict target
                update_clause = ", ".join([f"{key} = excluded.{key}" for key in keys if key != "file_path"])
